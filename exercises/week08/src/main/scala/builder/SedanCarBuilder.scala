...
object SedanCarBuilder extends CarBuilder {
  ...
  override def buildBodyStyle: CarBuilder = {
    car.setBodyStyle("""
    External dimensions:
      overall length (inches): 202.9
      overall width (inches): 76.2,
      overall height (inches): 60.7,
      wheelbase (inches): 112.9,
      front track (inches): 65.3,
  ...
  override def buildFuelType: CarBuilder = {
    car.setFuelType("Diesel 19 MPG city, 29 MPG highway, 23 MPG combined and " +
      "437 mi. range")
    this
  }
